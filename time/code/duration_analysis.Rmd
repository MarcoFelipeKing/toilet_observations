...
### Urination: Testing the time difference between women and men 
```{r}
t.test(data=df %>% filter(activity=="U"),log10(total_duration)~gender)
```
```
#Defecation: Testing the time difference between women and men
```{r}
t.test(data=df %>%
         filter(activity=="D"), log10(total_duration)~gender)
```
#Gender neutral toilets: Different in urination 
```{r}
df %>% ungroup() %>% 
  group_by(gender) %>%
  filter(toilet_type=="GN")
  t.test(data=df %>% 
           filter(activity=="U"), log10(total_duration)~gender)
