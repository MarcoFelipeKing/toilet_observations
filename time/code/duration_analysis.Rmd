...
```{r}
# Ensuring total duration is numeric 
df1 <- df1 %>%
  mutate(`Total duration` = as.numeric(`Total duration`)) %>%  
  
  #Calculate summary statistics 
  summary_stats <- df1 %>%
  group_by(Gender, activity, toilet_type) %>%
  summarise(
    Average = round(mean(`Total duration`, na.rm = TRUE), 2),
    `Standard Deviation` = round(sd(`Total duration`, na.rm = TRUE), 2),
    Variance = round(var(`Total duration`, na.rm = TRUE), 2),
    `Standard Error` = round(sd(`Total duration`, na.rm = TRUE) / sqrt(n()), 2),
    Median = round(median(`Total duration`, na.rm = TRUE), 2)
  ) %>%
  ungroup() %>%
  rename_with(str_to_title) %>% 
  kable(caption = "Summary statistics for duration based on activity, toilet type and sex") %>% 
...
#Comparing MHM in sex-segregated and gender neutral toilets
```{r}

m_activity_data <- df1 %>%
  filter(activity == "M") %>%
  group_by(toilet_type) %>%
  mutate(log_total_duration = log10(Total duration))  

# Conduct t-test comparing the log total durations between the two toilet types
t_test_results <- t.test(log_total_duration ~ toilet_type, data = m_activity_data)
